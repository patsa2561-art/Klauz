...
//
// PRIVACY: IPs are TRUNCATED to /24 (IPv4) / /48 (IPv6) before being written
// to disk. The raw IP is never persisted, so a breach of the log file cannot
// be used to identify individual users — only to count unique networks.
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { clientIp, truncateIp } from './security.js';
...
export function logVisit(req) {
  try {
    const entry = {
      ...
      ip: truncateIp(clientIp(req)),    // /24 only — never the full address
